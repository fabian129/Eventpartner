...
export function AiAssistantContent({ cms }: { cms?: AiCMS }) {
  ...
  const defaultFeatures = t.raw('features') as { icon: string; title: string; desc: string }[];
  const features = cms?.features || defaultFeatures;
  ...
  return (
    <main className="relative w-full pt-32 md:pt-44 pb-20 md:pb-32 overflow-hidden">
      ...
      {/* Bot Preview Card */}
      <section className="relative max-w-[1200px] mx-auto px-6 md:px-10 mb-24 md:mb-36">
        <motion.div initial={{ opacity: 0, y: 25 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.9, ease: EASE }} className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#111] to-[#1a1a1a] border border-white/[0.06] p-10 md:p-16">
          {/* Glow */}
          ...
          <div className="relative flex flex-col items-center text-center">
            ...
            <h2 className="font-display text-3xl md:text-4xl font-semibold text-white mb-4">
              {cms?.botTitle || t('botTitle')}
            </h2>
            <p className="text-white/60 text-lg max-w-lg mb-8">
              {cms?.botDescription || t('botDescription')}
            </p>
            ...
            <div className="flex items-center gap-3 bg-white/[0.06] border border-white/[0.08] rounded-2xl px-6 py-4 mb-6">
              ...
              <span className="text-white/50 text-sm font-mono">{cms?.botStatus || t('botStatus')}</span>
            ...
            <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-white/30">{cms?.botLaunchDate || t('botLaunchDate')}</span>
          </div>
        </motion.div>
      </section>
      ...
      <section className="relative max-w-[1200px] mx-auto px-6 md:px-10 mb-24 md:mb-36">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: EASE }} className="mb-14">
          <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-tiffany block mb-6">{cms?.featuresLabel || t('featuresLabel')}</span>
          <h2 className="font-display text-3xl md:text-4xl font-medium tracking-tight text-[var(--text-primary)] leading-[0.95] max-w-lg">{cms?.featuresHeadline || t('featuresHeadline')}</h2>
        </motion.div>
