    ...
    <main className="relative w-full pt-32 md:pt-44 pb-20 md:pb-32 overflow-hidden">
      ...
      <section className="relative max-w-[1200px] mx-auto px-6 md:px-10 mb-24 md:mb-36">
        ...
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: EASE }} className="font-display text-[clamp(2.5rem,7vw,5.5rem)] font-bold uppercase tracking-[-0.02em] text-[var(--text-primary)] leading-[0.88] mb-10">
          {cms?.heroHeadline || "Your intelligent"}<br /><span className="text-[#6AD8D2]">{cms?.heroHeadlineAccent || "event partner."}</span>
        </motion.h1>
      ...
      <section className="relative max-w-[1200px] mx-auto px-6 md:px-10 mb-24 md:mb-36">
        <motion.div initial={{ opacity: 0, y: 25, filter: "blur(8px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }} viewport={{ once: true }} transition={{ duration: 0.9, ease: EASE }} className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#111] to-[#1a1a1a] border border-white/[0.06] p-10 md:p-16">
          ...
          <div className="relative flex flex-col items-center text-center">
            <motion.div initial={{ scale: 0.8, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2, ease: EASE }} className="w-24 h-24 rounded-2xl bg-[#6AD8D2]/10 border border-[#6AD8D2]/20 flex items-center justify-center mb-8">
              <Bot className="w-12 h-12 text-[#6AD8D2]" />
            </motion.div>
            ...
            <div className="flex items-center gap-3 bg-white/[0.06] border border-white/[0.08] rounded-2xl px-6 py-4 mb-6">
              <Sparkles className="w-5 h-5 text-[#6AD8D2] animate-pulse" />
              <span className="text-white/50 text-sm font-mono">Training in progress...</span>
              <div className="flex gap-1 ml-2">
                <span className="w-1.5 h-1.5 bg-[#6AD8D2] rounded-full animate-bounce" style={{ animationDelay: "0s" }} />
                <span className="w-1.5 h-1.5 bg-[#6AD8D2] rounded-full animate-bounce" style={{ animationDelay: "0.15s" }} />
                <span className="w-1.5 h-1.5 bg-[#6AD8D2] rounded-full animate-bounce" style={{ animationDelay: "0.3s" }} />
              </div>
            </div>
      ...
      <section className="relative max-w-[1200px] mx-auto px-6 md:px-10 mb-24 md:mb-36">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7, ease: EASE }} className="mb-14">
          <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-[#6AD8D2] block mb-6">Capabilities</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {FEATURES.map((f, i) => (
            <motion.div key={f.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.08 * i, ease: EASE }} className="p-8 rounded-2xl bg-[var(--bg-card)] border border-[var(--border-default)] hover:border-[#6AD8D2]/20 hover:shadow-lg hover:shadow-[#6AD8D2]/[0.03] transition-all duration-300">
              <div className="w-10 h-10 rounded-xl bg-[#6AD8D2]/10 flex items-center justify-center mb-5"><f.icon className="w-5 h-5 text-[#6AD8D2]" /></div>
