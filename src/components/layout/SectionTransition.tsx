    ...
    return (
      <div className="relative py-12 md:py-16 w-full flex flex-col items-center justify-center overflow-hidden gap-3">
        <motion.div
          ...
          className="w-2.5 h-2.5 bg-[#81D8D0]/40 rounded-[1px]"
        />
        <motion.div
          initial={{ width: "0%" }}
          whileInView={{ width: "100%" }}
          ...
          className="h-[1px] max-w-[120px] bg-gradient-to-r from-transparent via-[#81D8D0]/20 to-transparent"
        />
      </div>
    );
  }
        ...
        {[0, 1, 2].map((i) => (
          <motion.div
            ...
            className={`rounded-full ${
              i === 1
                ? "w-2 h-2 bg-[#81D8D0]/50"
                : "w-1.5 h-1.5 bg-[#81D8D0]/25"
            }`}
          />
        ))}
      </div>
